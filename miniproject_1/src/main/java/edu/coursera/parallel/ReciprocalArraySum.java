...
public final class ReciprocalArraySum {
  ...
  private static class ReciprocalArraySumTask extends RecursiveAction {
    /**
     * Threshold for deciding the recursive action.
     */
  ...
  protected static double parArraySum(final double[] input) {
    ...
    // Compute the reciprocal sum in parallel
    // by dividing the array into two and doing
    // it in parallel.
  ...
  protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
    ...
    int numElementsInEachChunk = input.length / numTasks;
    /*
     * System.out.println("Number of chunks for " + input.length +
     * " elements and number of tasks - " + numTasks + " is " + numElementsInEachChunk);
     */
    // Create tasks for each chunk and add to a list of tasks.
    for (int i = 0; i < numTasks; i++) {

      int start = getChunkStartInclusive(i, numTasks, input.length);
      int end = getChunkEndExclusive(i, numTasks, input.length);
      // System.out.println("Iteration - " + i + " Start - " + start + " End - " + end);
      ReciprocalArraySumTask task = new ReciprocalArraySumTask(start, end, input);
      tasks.add(task);
    }
    ReciprocalArraySumTask.invokeAll(tasks);
    
    // Take each task and join to be added to the result.
