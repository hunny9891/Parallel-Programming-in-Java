...
public final class OneDimAveragingPhaser {
   ...
   */
  public static void runSequential(final int iterations, final double[] myNew, final double[] myVal,
      final int n) {
    double[] next = myNew;
    double[] curr = myVal;
   ...
   */
  public static void runParallelBarrier(final int iterations, final double[] myNew,
      final double[] myVal, final int n, final int tasks) {
  ...
  public static void runParallelFuzzyBarrier(final int iterations, final double[] myNew,
    ...
    for (int ii = 0; ii < tasks; ii++) {
      System.out.println("Execuring iteration: " + ii);
      final int i = ii;
      threads[ii] = new Thread(() -> {
        double[] threadedMyVal = myVal;
        double[] threadedMyNew = myNew;

        for (int iter = 0; iter < iterations; iter++) {
          int left = i * (n / tasks) + 1;
          threadedMyNew[left] = (threadedMyVal[left - 1] + threadedMyVal[left + 1]) / 2.0;

          int right = (i + 1) * (n / tasks);
          threadedMyNew[right] = (threadedMyVal[right - 1] + threadedMyVal[right + 1]) / 2.0;

          // signal the completion of the phase.
          int currentPhase = phaser.arrive();

          for (int j = left + 1; j <= right - 1; j++) {
            threadedMyNew[j] = (threadedMyVal[j - 1] + threadedMyVal[j + 1]) / 2.0;
          }

          // wait for previous phase to complete before advancing.
          phaser.awaitAdvance(currentPhase);

          double[] temp = threadedMyNew;
          threadedMyNew = threadedMyVal;
          threadedMyVal = temp; 
        }
